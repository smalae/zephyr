				     ...
				     uint32_t channel)
{
	...
	for (int i = 0; i < config->block_count; i++) {
		...
		if (RSI_GPDMA_BuildDescriptors(GPDMA_HANDLE, xfer_cfg,
				   ...
				   struct dma_config *config)
{
	...
	if (cfg->reg->GLOBAL.DMA_CHNL_ENABLE_REG & BIT(channel)) {
				    ...
				    struct dma_status *stat)
{
	...
	stat->busy = cfg->reg->GLOBAL.DMA_CHNL_ENABLE_REG & BIT(channel) ? true : false;
...
#define SIWX91X_GPDMA_INIT(inst)                                                                   \
	...
	__aligned(32) static struct gpdma_siwx91x_data gpdma_data_##inst = {                       \
